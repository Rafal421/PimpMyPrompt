...
export async function loginUser(email: string, password: string): Promise<ActionResult | void> {
  ...
  try {
    ...
    if (error) {
      await AuditLogger.log("LOGIN_FAILED", validation.data.email, { error_type: error.message });
...
export async function signupUser(email: string, password: string): Promise<ActionResult | void> {
  ...
  try {
    ...
    if (error) {
      await AuditLogger.log("SIGNUP_FAILED", validation.data.email, { error_type: error.message });
...
export async function resetPassword(email: string): Promise<ActionResult> {
  ...
  try {
    ...
    if (error) {
      await AuditLogger.log("PASSWORD_RESET_FAILED", email, { error_type: error.message });
      return {
        success: false,
        error: mapSupabaseError(error),
      };
...
export async function updatePassword(password: string): Promise<ActionResult> {
  ...
  try {
    ...
    if (error) {
      if (user) {
        await AuditLogger.log("PASSWORD_UPDATE_FAILED", user.id, { error_type: error.message });
      }
      return {
        success: false,
        error: error.message,
